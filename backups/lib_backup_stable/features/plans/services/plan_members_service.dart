...
class PlanMembersService {
  ...
  Future<List<PlanMember>> getMembers(String planId) async {
    try {
      ...
      for (var row in membersRes) {
        ...
        try {
           final profile = await _supabase.from('profiles').select('display_name, email').eq('id', uid).maybeSingle();
           if (profile != null) {
        ...
        members.add(PlanMember(id: uid, name: displayName, isGuest: false, role: role));
      }
  ...
  Future<String> getMyRole(String planId) async {
       ...
       return res?['role'] as String? ?? 'member';
  }
}
