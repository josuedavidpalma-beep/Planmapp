...
class _PlanDetailScreenState extends State<PlanDetailScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _tabController = TabController(length: 4, vsync: this); // 4 Tabs (Resumen, Logística, Gastos, Juegos)
      ...
      body: _isLoading 
        ...
        : NestedScrollView(
            headerSliverBuilder: (context, innerBoxIsScrolled) => [
              SliverAppBar(
                title: Text(_plan?.title ?? "Plan Details", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
                pinned: true,
                floating: true,
        ...
        content: StatefulBuilder(
          builder: (context, setDialogState) => Column(
              ...
              children: [
                ...
                TextButton.icon(
                  onPressed: () async {
                      if (titleController.text.isEmpty) return; // Need a question
                      // Show loading indicator or toast?
                      // Ideally we'd have state in the dialog for loading.
                      // For quick win: show snackbar 'Thinking...' and then update
                      ...
                      try {
                          ...
                          final response = await supabase.functions.invoke('ai-assistant', body: {
                               ...
                               'payload': { 
                                   ...
                                   // We could pass Plan location if we had it easily accessibile, defaults to general
                               }
                          });
                          ...
                          if (errorMsg.contains("Exception:")) errorMsg = errorMsg.replaceAll("Exception:", "").trim();
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Error AI: $errorMsg")));
                      }
                  },
                  icon: const Icon(Icons.auto_awesome, color: Colors.purple),
                  label: const Text("Sugerir Opciones", style: TextStyle(color: Colors.purple)),
                ),
                const SizedBox(height: 16),
                ListTile(
