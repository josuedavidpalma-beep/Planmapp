...
class ItineraryPlanTab extends StatefulWidget {
  ...
  const ItineraryPlanTab({super.key, required this.planId, required this.userRole});
...
class _ItineraryPlanTabState extends State<ItineraryPlanTab> {
      ...
      return Center(
            child: Column(
                ...
                children: [
                    ...
                    const SizedBox(height: 16),
                    const Text("No hay actividades aún", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    const Text("Agrega eventos o visualiza el mapa.", style: TextStyle(color: Colors.grey)),
                ],
            )
        );
  }
          ...
          itemBuilder: (context, index) {
              ...
              return Card(
                  ...
                  child: ListTile(
                      ...
                      trailing: _canEdit() ? IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _deleteActivity(activity.id)) : null,
                  ),
              );
          },
      );
  ...
  void _openAddActivity() async {
      final result = await Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => AddActivityScreen(planId: widget.planId)),
      );
      if (result == true) {
          _loadActivities();
      }
