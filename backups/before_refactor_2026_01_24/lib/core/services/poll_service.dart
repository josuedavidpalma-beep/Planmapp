...
class PollService {
        ...
        .asyncMap((event) async {
          ...
          for (var pollData in event) {
             ...
             for (var opt in optionsRes) {
                ...
                opt['poll_votes'] = {
                  'count': votes.length,
                  'user_id': votes.any((v) => v is Map && v['user_id'] == currentUserId) ? currentUserId : null
                };
             }
  ...
  Future<void> promotePollToActivity(String pollId) async {
      ...
      // 3. Update Plan Metadata if applicable (Location/Date)
      final qLower = question.toLowerCase();
      if ((qLower.contains('lugar') || qLower.contains('dónde') || qLower.contains('donde')) && winner['text'] != null) {
          try {
             await _supabase.from('plans').update({'location': winner['text']}).eq('id', planId);
          } catch (_) {}
      } else if ((qLower.contains('fecha') || qLower.contains('cuándo') || qLower.contains('cuando')) && activity.startTime != null) {
           // If it was a date poll, try to parse winner or use activity start time
           try {
             // We can only reliably update if the winner text IS a date, or rely on the Activity Start Time we just calculated
             // Let's use the calculated start time from the Activity logic (which might need improvement for Date polls)
             // Simpler: Just update event_date if we have a valid date string in winner or activity
             await _supabase.from('plans').update({'event_date': activity.startTime.toIso8601String()}).eq('id', planId);
           } catch (_) {}
      }
