...
class _DebtsDashboardScreenState extends State<DebtsDashboardScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildList(List<Map<String, dynamic>> items, bool isReceivable) {
      ...
      return ListView.builder(
          ...
          itemBuilder: (context, index) {
              return isReceivable 
                  ? _buildReceivableCard(items[index]) 
                  : _buildPayableCard(items[index]);
          },
      );
  }
  ...
  Widget _buildReceivableCard(Map<String, dynamic> debt) {
      ...
      return Card(
          elevation: isReported ? 6 : 2,
          shadowColor: isReported ? Colors.orange.withOpacity(0.4) : null,
          margin: const EdgeInsets.only(bottom: 12),
          shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(16),
              side: isReported ? const BorderSide(color: Colors.orange, width: 2) : BorderSide.none,
          ),
          child: Padding(
  ...
  Widget _buildPayableCard(Map<String, dynamic> debt) {
      // For payables, the creditor is expenses.created_by
      // In the modified repository query, the profile is mapped to profiles (due to the foreign key on created_by)
      // Wait, we used `profiles!expenses_created_by_fkey` in the repo but Supabase returns it as the alias or table name.
      // Usually it returns it as `profiles`.
      final profileData = debt['profiles'] as Map<String, dynamic>?;
            ...
            child: Row(
              children: [
                  ...
                  Column(
                      crossAxisAlignment: CrossAxisAlignment.end,
                      children: [
                          Text(_currencyFormat.format(amount), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.red)),
                          const SizedBox(height: 8),
                          if (isReported)
                              const Padding(
