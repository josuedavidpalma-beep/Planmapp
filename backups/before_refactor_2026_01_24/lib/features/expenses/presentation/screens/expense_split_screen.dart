...
class _WizardSheetState extends State<_WizardSheet> with SingleTickerProviderStateMixin {
  ...
  void initState() {
      ...
      // Initialize with current quantities (defaulting to Partes mode)
      _initValuesFromCurrent();
  }

  void _initValuesFromCurrent() {
      for (var m in widget.members) {
          final found = widget.currentAssignments.firstWhere((a) => a.userId == m.id, orElse: () => const AssignmentModel(quantity: 0));
          _tempValues["u_${m.id}"] = found.quantity;
      }
      for (var g in widget.guests) {
          final found = widget.currentAssignments.firstWhere((a) => a.guestName == g, orElse: () => const AssignmentModel(quantity: 0));
          _tempValues["g_$g"] = found.quantity;
      }
  }

  void _applyEqual() {
      final total = widget.members.length + widget.guests.length;
      if (total == 0) return;
      final val = widget.item.quantity / total;
      setState(() {
          for(var key in _tempValues.keys) _tempValues[key] = val;
      });
  }

  void _save() {
      List<AssignmentModel> result = [];
      // Logic depends on Tab? No, we should rely on what's in _tempValues based on the "Mode" logic.
      // Actually, updating modes is tricky. Let's simplify: 
      // The wizard calculates "Quantity" (the universal storage) based on input.
      
      // If Tab 1 (Partes): Values ARE quantities.
      // If Tab 2 (Percent): Values are %. Qty = (Val/100) * ItemQty.
      // If Tab 3 (Amount): Values are $. Qty = Val / UnitPrice.
      
      final unitPrice = widget.item.price / widget.item.quantity; // Price per 1 qty

      _tempValues.forEach((key, val) {
          if (val <= 0) return;
          
          double finalQty = 0;
          if (_tabController.index == 0) { // Partes
              finalQty = val;
          ...
          child: Column(
              ...
              children: [
                  ...
                  TabBar(
                      ...
                      onTap: (index) {
                          // When switching tabs, we should probably reset or try to convert?
                          // Converting is nicer.
                          setState(() {
                              final unitPrice = widget.item.price / widget.item.quantity;
                              final newValues = <String, double>{};
                              
                              _tempValues.forEach((k, v) {
                                  // Convert v (current mode unit) back to Qty first
                                  double asQty = 0;
                                  // PREV MODE was.. impossible to know without state tracking. 
                                  // Simplification: Always Reset to 0 when switching tabs OR assume previous values imply intent.
                                  // Better: Recalculate from clean slate helps avoid confusion. 
                                  // Use 0.
                                  newValues[k] = 0.0;
                              });
                              _tempValues.clear();
